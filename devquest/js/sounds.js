  ...
  const api = {
    ...
    introTypewriter() {
      play(actx => {
        ...
        pitches.forEach((freq, i) => tone(actx, freq, 0.022, 0.055, 'square', 0.08 + i * 0.065));
    ...
    keyClick(correct = true) {
      play(actx => {
        if (correct) tone(actx, 1080, 0.022, 0.038, 'square');
        else { tone(actx, 220, 0.05, 0.055, 'sawtooth'); noise(actx, 0.04, 0.03, 0.01); }
      });
    },
    ...
    autocompleteAppear() {
      play(actx => sweep(actx, 550, 850, 0.14, 0.07, 'sine'));
    },
    ...
    timerWarning() {
      play(actx => tone(actx, 660, 0.07, 0.1, 'sine'));
    },
    ...
    gamePass() {
      play(actx => {
        [523, 659, 784, 1047].forEach((freq, i) => tone(actx, freq, 0.45, 0.14, 'sine', i * 0.13));
      });
    },

    gameFail() {
      play(actx => {
        [392, 330, 262, 220].forEach((freq, i) => tone(actx, freq, 0.4, 0.11, 'sine', i * 0.18));
      });
    },
    ...
    correctDrop() {
      play(actx => {
        tone(actx,  160, 0.18, 0.22, 'sine', 0);
        noise(actx, 0.06, 0.08, 0.02);
        tone(actx, 1000, 0.12, 0.10, 'sine', 0.06);
      });
    },
    ...
    allSorted() {
      play(actx => {
        [523, 659, 784, 659, 1047].forEach((freq, i) => tone(actx, freq, 0.35, 0.16, 'sine', i * 0.11));
      });
    },

    lifeLost() {
      play(actx => sweep(actx, 460, 200, 0.35, 0.14, 'sine'));
    },

    terminalClick() {
      play(actx => tone(actx, 820 + Math.random() * 320, 0.018, 0.042, 'square'));
    },
    ...
    themeToggle(toLight) {
      play(actx => toLight
        ? sweep(actx, 700, 1100, 0.14, 0.08, 'sine')
        : sweep(actx, 550,  280, 0.18, 0.09, 'sine')
      );
    },
  };
