...
export function Footer() {
  ...
  return (
    <footer
      id="contact"
      className="bg-paper-400 border-t-4 border-black px-6 py-20 md:px-12 md:py-28"
    >
      <div className="mx-auto max-w-5xl">
          ...
          <nav aria-label="Elsewhere" className="mt-12 flex flex-wrap gap-3">
            {SOCIALS.map((s) => (
              <a
                ...
                className="font-mono border-2 border-black bg-card px-4 py-2 text-sm shadow-sm transition-[transform,box-shadow] duration-[var(--dur-fast)] hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
              >
                {s.label}
                {s.external && ' ↗'}
              </a>
        ...
        <p className="font-pixel mt-16 text-[0.65rem] tracking-widest uppercase">
      ...
      <AnimatePresence>
        {victimNumber !== null && (
          <motion.div
            role="status"
            className="fixed bottom-6 left-1/2 z-50 w-[min(92vw,26rem)] -translate-x-1/2 border-2 border-black bg-primary p-4 shadow-xl"
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <p className="text-sm">
                ...
                <span className="font-head tabular-nums">#{victimNumber}</span>. Never gonna give
                you up.
              </p>
              <button
                type="button"
                onClick={dismiss}
                aria-label="Dismiss"
                className="font-head shrink-0 border-2 border-black bg-card px-2 leading-tight focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
              >
                ×
              </button>
            </div>
